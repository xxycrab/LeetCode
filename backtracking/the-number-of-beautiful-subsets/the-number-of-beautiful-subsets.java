class Solution {
    ...
    public int beautifulSubsets(int[] nums, int k) {
        ...
        for (Map.Entry<Integer, Map<Integer, Integer>> entry : freqMap.entrySet()) {
            ...
            totalCount *= countBeautifulSubsets(subsets, k, 0);
        }

        return totalCount - 1;
    }

    private int countBeautifulSubsets(ArrayList<Pair<Integer, Integer>> subsets, int difference, int i) {
        ...
        int skip = countBeautifulSubsets(subsets, difference, i + 1);
        ...
        if (i + 1 < subsets.size() && subsets.get(i + 1).getKey() - subsets.get(i).getKey() == difference) {
            take *= countBeautifulSubsets(subsets, difference, i + 2);
        } else {
            take *= countBeautifulSubsets(subsets, difference, i + 1);
        }
